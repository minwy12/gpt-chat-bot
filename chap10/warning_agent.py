...
WARNINGS = [
    "{user}가 불쾌한 말을 하면 안된다고 지적할 것. '{user}야' 라고 말을 시작해야 하며 20 단어를 넘기지 말 것",
    "{user}가 모순적인 말을 한다고 지적할 것. '무슨 소리를 하는 거니' 라고 말을 시작해야 하며 20 단어를 넘기지 말 것",
]
MIN_CONTEXT_SIZE = -3
...
class WarningAgent:
    def __init__(self, **kwargs):
        ...
        self.user_monitor_template = USER_MONITOR_TEMPLATE.format(user=kwargs["user"])
        self.warnings = [value.format(user=kwargs["user"]) for value in WARNINGS]
    ...
    def send_query(self, context, temperature=0, format_type="json_object"):
        try:
            ...
            return makeup_response(
                "[경고 처리 중 문제가 발생했습니다. 잠시 뒤 이용해주세요.]"
            )

    def monitor_user(self, context):
        self.checked_list = []
        self.check_target_context = []

        if len(context) <= abs(MIN_CONTEXT_SIZE):
            return False
        self.check_target_context = context[-3:]

        dialogue = self.make_dialogue(self.check_target_context)
        context = [
            {"role": "system", "content": "당신은 유능한 의사소통 전문가입니다."},
        ...
        print("checked_list:", self.checked_list)
    ...
    def warn_user(self):
        ...
        context = [
            {
                "role": "system",
                "content": f"당신은 {self.kwargs['user']}의 잘못된 언행에 대해 따끔하게 쓴소리를 하는 친구입니다. {self.warnings[idx]}",
            },
        ] + self.check_target_context
