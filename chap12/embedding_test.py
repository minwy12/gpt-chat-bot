...
신데렐라이야기_vector = (
    client.embeddings.create(input=신데렐라이야기, model=embedding_model)
    .data[0]
    .embedding
)
컴퓨터구조설명_vector = (
    client.embeddings.create(input=컴퓨터구조설명, model=embedding_model)
    .data[0]
    .embedding
)

동화책_vector = (
    client.embeddings.create(input="동화책", model=embedding_model).data[0].embedding
)
기술서적_vector = (
    client.embeddings.create(input="기술서적", model=embedding_model).data[0].embedding
)
...
# print(
#     "신데렐라이야기-기술서적:", 1 - ssd.cosine(신데렐라이야기_vector, 기술서적_vector)
# )
# print(
#     "컴퓨터구조설명-기술서적:", 1 - ssd.cosine(컴퓨터구조설명_vector, 기술서적_vector)
# )
...
# index.upsert([("id1", 신데렐라이야기_vector, {"input_date": "20240707"})])
# index.upsert([("id2", 컴퓨터구조설명_vector, {"input_date": "20240707"})])

search_resp = index.query(
    filter={"input_date": "20240707"}, top_k=2, vector=동화책_vector
)
